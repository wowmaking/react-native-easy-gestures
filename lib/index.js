...
export default class Gestures extends Component {
  ...
  componentDidMount() {
    const { style, scale, rotation, drag } = this.state;
    ...
    if (rotation) {
      this.pinchStyles.transform.push({rotate: rotation});
    }
    if (drag) {
      this.dragStyles = drag;
    }
    if (scale || rotation || drag) {
      this.updateStyles();
    }
  }
  ...
  onDrag(event, gestureState) {
    ...
    const { draggable, dragBorders } = this.props;
    ...
    if(!dragBorders){
      return this.dragStyles = {left, top};
    }

    if ((dragBorders.widthMin > left ||
        dragBorders.widthMax < left) &&
        (dragBorders.heightMin > top ||
            dragBorders.heightMax < top)) {
      return null;
    }
    ...
    if (dragBorders.widthMin > left ||
        dragBorders.widthMax < left) {
      this.dragStyles = {top};
    } else if (dragBorders.heightMin > top ||
        dragBorders.heightMax < top) {
      this.dragStyles = {left};
    } else {
